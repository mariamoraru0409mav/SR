...
def upload_users(recombee_client: RecombeeClient, csv_path):
    users = []
    ...
    try:
        recombee_client.send(Batch(users))
        print(f"Users upload complete ({len(users)} users).")
    except Exception as err:
        print(f"Error uploading users: {err}")


def add_user_properties(recombee_client: RecombeeClient):
    try:
        properties = [
            AddUserProperty("gender", "string"),
            AddUserProperty("birthDate", "string"),
            AddUserProperty("firstName", "string"),
            AddUserProperty("lastName", "string"),
            AddUserProperty("patientId", "string"),
        ]

        recombee_client.send(Batch(properties))
        print("User properties added successfully.")
    except Exception as err:
        print(f"Error setting user properties: {err}")
...
if __name__ == "__main__":
    ...
    add_user_properties(recombee_client)
    upload_appointments(recombee_client, "healthcare_noshows.csv")
    upload_users(recombee_client, "users.csv")
